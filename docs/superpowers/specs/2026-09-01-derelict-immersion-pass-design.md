...
- **§4.3 the ritual tick and the window:** `ritual.phase` outlives `endsAt` in the store by design (the scenes apply `ritualExpired` against the clock; only `armRitual`/`confirmRitual` rewrite the phase), so `mixFor(state, now)` gates `ritualTick` on the window as well as on `armed`, and `startMixer`'s one-second interval re-evaluates it and clears itself — an abandoned ritual falls silent when its window closes. Every future consumer of `ritual.phase` must apply the same wall-clock check.
- **§4.3–4.4 the compressor and the ship settling (Sep 2, after Mario's first listen):** the cryo bay's deep slow swell (±50 % at 0.15 Hz) read as surf, not machinery. The layer is now a near-steady hiss (base 0.02, a 10 % flutter) with a mechanical cycle — a pneumatic puff (one second up, two down) then the tick, every 8–14 s. `mixFor` gains `awake` (0 dormant, 0.5 on aux power, 1 with the engines online): the compressor settles as the ship wakes (level ×0.4, band 2 kHz → 1.2 kHz, puffs every 14–24 s) and the fan/vent layers of the medbay, crew quarters and hydroponics drop up to 30 %. The ambience filter at rest opens by chapter — 8 kHz, 10 kHz, 12 kHz — so the ship grows more present the deeper the crew goes; the wave values are unchanged.
- **§5.2 and the ritual chrome (Sep 7, pre-merge review of the cinematic cut):** the bulkhead sounds on every room change — motion preferences (OS or the in-app SUBTLE toggle) govern motion only, so the reduced path swaps instantly but is no longer silent. Every lazy scene chunk sits under a `SceneBoundary` that reports a failed load and offers to reconnect instead of unmounting the app. `ritualLive(ritual, id, now)` in `ritual.ts` and the `useRitualLive(id)` hook are the one way scene chrome may show ARMED: the store leaves `phase` at `'armed'` when a window lapses. The director's arrival accent no longer stamps a 12 s discovery blackout (only discoveries space themselves that far; impact and release keep 5 s), and the kill-switch stirring / chapter-3 turn sound the alarm again. Cleanup in the same pass: `RoomPlate` replaces the `<picture>` block pasted in ten scenes; `inspect()` in `src/ui/inspect.ts` replaces nine station-jump copies; dead `Machinery.tsx`, its CSS and the frost helpers of the old thaw are gone; `MachineryCue` no longer names wave/recover/containment (the director sounds them); the patch bay keeps the full wiring state readable while a lead is selected and a second click deselects; the epilogue renders inside `Atmosphere` so SUBTLE stills its reflection; the cryo memory print uses a 128 px derivative of the family photo.
