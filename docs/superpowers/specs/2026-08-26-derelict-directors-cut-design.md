...
**Plan C (Chapter 3) shipped Aug 30, 2026** — merged to `main` and deployed (190 tests, 29 tools). The Director's Cut is complete: three chapters, three joint-ritual endings (LEAVE / RESTORE / BROADCAST) with `ending` as the epilogue discriminator, and the kill-switch as a pure engine (`src/game/killswitch.ts`) whose suppression composes into every tool's availability — the registry never changed. Deviations recorded: narrative stays in one module (§7.3's per-chapter split deferred); `adjust_atmosphere` (§4) not built; isolation power is held for good once a bus is shielded (the "power cost elsewhere" is enforced, not fiction); waves are always telegraphed — a resumed save restarts its cycle from calm and a throttled clock can never jump straight into a wave.

**Plan D (Polish) shipped Aug 30, 2026** — one instrument palette (`--steel*`, `--face*`, `--brass*`, `--parchment` tokens replace the scene literals, pinned by a test), hydroponics bed lamps report the last cycle the AI ran (`chapter2.lastCycle`), `wavesEndured` counts active→calm transitions and reaches the epilogue, the memory rack holds one of each column (198 tests, 29 tools). Voice-over brief for Okafor's recorder in `docs/audio/okafor-recorder-vo.md` (§7.6 open question — assets pending).
