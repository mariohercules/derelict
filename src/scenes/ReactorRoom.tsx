      ...
      <svg viewBox="0 0 380 150" width="100%" style={{ maxWidth: 560, display: 'block' }} role="img" aria-label={t.reactor.bankAria}>
        <defs>
          ...
          <linearGradient id="rr-brass" x1="0" y1="0" x2="1" y2="1">
            ...
            <stop offset="50%" stopColor="var(--brass-mid)" />
            <stop offset="100%" stopColor="var(--brass-lo)" />
          </linearGradient>
        </defs>
        {/* panel bezel + inset face */}
        <rect x="4" y="4" width="372" height="142" rx="6" fill="var(--face)" stroke="var(--steel)" strokeWidth="3" />
        ...
        {BUSES.map((bus, i) => {
          ...
          return (
            <g key={bus}>
              {/* lamp */}
              <circle cx={x + 32} cy="26" r="5" fill={cut ? 'var(--red)' : 'var(--steel-lo)'} stroke="var(--steel)" strokeWidth="1.5" />
              {cut && <circle cx={x + 32} cy="26" r="9" fill="var(--red)" opacity="0.18" />}
              {/* hinge block + blade: the lever rotates down when cut */}
              <rect x={x + 8} y="44" width="12" height="52" rx="2" fill="url(#rr-brass)" stroke="var(--brass-lo)" />
              <g className="lever" style={{ transform: cut ? 'rotate(58deg)' : 'rotate(0deg)', transformOrigin: `${x + 14}px 90px` }}>
                <rect x={x + 12} y="46" width="46" height="8" rx="2" fill="url(#rr-brass)" stroke="var(--brass-lo)" />
                <rect x={x + 50} y="42" width="10" height="16" rx="2" fill="var(--face)" stroke="var(--steel)" />
              </g>
              <circle cx={x + 14} cy="90" r="4" fill="var(--face)" stroke="var(--brass-lo)" strokeWidth="1.5" />
              {/* contact jaw */}
              <rect x={x + 54} y="86" width="10" height="14" rx="1.5" fill="var(--steel-lo)" stroke="var(--steel)" />
              {/* engraved bus plate */}
              <rect x={x + 6} y="112" width="64" height="16" rx="2" fill="var(--panel-solid)" stroke="var(--line)" />
